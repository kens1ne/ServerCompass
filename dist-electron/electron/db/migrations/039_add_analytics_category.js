...
function migrate(db) {
    console.log('[Migration 039] Adding analytics category to docker_compose_templates');
    // SQLite doesn't support modifying CHECK constraints directly
    // We need to recreate the table with the new constraint
    // 1. Create new table with updated CHECK constraint (includes analytics)
    db.exec(`
    CREATE TABLE IF NOT EXISTS docker_compose_templates_new (
      id TEXT PRIMARY KEY,
      name TEXT NOT NULL,
      description TEXT,
      category TEXT NOT NULL CHECK(category IN ('nextjs', 'express', 'nestjs', 'static', 'python', 'go', 'database', 'fullstack', 'custom', 'cms', 'nocode', 'analytics')),
  ...
  `);
    console.log('[Migration 039] Added analytics category to docker_compose_templates');
}
//# sourceMappingURL=039_add_analytics_category.js.map